...
<meta property="og:id" content="{{$app_query->id}}" />
...
<!-- Grid column -->
          ...
          <div class="d-flex flex-row app-info">
            <div class="mr-2">
                <img src="{{ asset('images') }}/{{$app_query->image}}" href="{{ asset('images') }}/{{$app_query->image}}" loading="lazy" class="float-left pimage" alt="{{$app_query->slug}}">
                    </div>
                       <div>
                ...
                <div class="container screenshots mt-3">
                    ...
                    <div id="screenshot-main">
                            ...
                            <a href="{{ asset('screenshots') }}/{{$image_name}}"><img src="{{ asset('screenshots') }}/{{$image_name}}"  loading="lazy" class="mr-1" alt="{{$app_query->slug}}"></a>
                            @endforeach
                        </div>
                    </div>
                </div>
...
<!-- Other Apps in This Category -->
                <div class="d-flex top-title justify-content-between">
                    ...
                    <div>
                <a href="{{ asset('category') }}/{{ $data_to_pass['category_slug'] }}">@lang('general.more')»</a></div>
                </div>
                 <div class="featured-apps mr-3 ml-3">
                    <div class="row flex-nowrap page-left mb-1 pb-2">
                     ...
                     <div class="col-half p-2 @if($loop->last) mr-0 @endif">
                        <a href="{{ asset($settings['app_base']) }}/{{ $app_category->slug }}">
                        <img src="{{ asset('images') }}/{{$app_category->image}}" loading="lazy" class="img-fluid rounded " alt="{{ $app_category->slug }}" >
         ...
         <div id="move_item"></div>
               <div id="download_section">
                ...
                <div class="show-qr-code mt-2"><i class="fas fa-qrcode"></i>
                        ...
                        @lang('general.qr_license')
                        @endif
                        <div class="qr-code">
                        <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->margin(0)->size(125)->generate(url()->current())); !!}" loading="lazy" alt="{{$app_query->slug}}">
                    </div>
                </div>
<ul class="specs">
  <table class="table table-bordered">
    ...
    <tbody>
     ...
     <tr>
      ...
      <tr>
        <td><strong>@lang('general.license')</strong></td>
        <td>{{$app_query->license}}</td>
      </tr>
      <tr>
        <td><strong>@lang('general.votes')</strong></td>
        <td>{{$app_query->total_votes}}</td>
      </tr>
      <tr>
        <td><strong>@lang('general.rating')</strong></td>
        <td>{{$app_query->votes}}</td>
      </tr>
     <tr>
        ...
        <div class="d-flex top-title justify-content-between mt-md-3" id="popular_apps">
            ...
            <div class="featured-apps app-page mr-3 ml-3">
                    <div class="row flex-nowrap mb-1 pb-2">
                     ...
                     <div class="col-half p-2 @if($loop->last) mr-0 @endif">
                            <a href="{{ asset($settings['app_base']) }}/{{ $app_category->slug }}">
                            <img src="{{ asset('images') }}/{{$app_category->image}}" class="img-fluid rounded" loading="lazy" alt="----------{{ $app_category->slug }}">
                            <span>{{$app_category->title}}</span></a>
                        </div>
                        @endforeach
                    </div>
